...
export const MapScreen: React.FC<MapScreenProps> = ({ routeSelection, onSelectionCleared }) => {
  ...
  useEffect(() => {
    // The subscription now handles simulation logic internally within transportService
  ...
  const visibleVehicles = useMemo(() => {
    // The transportService subscription returns filtered vehicles if routeId is passed,
    // but we filter here again to be safe if the subscription returns all.
    if (!selectedRouteId) return [];
    return vehicles.filter(v => v.route_id === selectedRouteId);
  }, [vehicles, selectedRouteId]);
            ...
            <div class="p-2 font-sans">
                ...
                <div class="grid grid-cols-2 gap-2 text-xs border-t border-neutral-200 pt-2">
                     <div>
                        ...
                        <div class="font-mono font-bold">${vehicle.vehicle_id.replace('GTFS-', '').replace('SIM-', '')}</div>
                     </div>
