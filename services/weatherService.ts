...
export async function getWeatherForLocation(
  ...
  url.searchParams.set(
    "hourly",
    "temperature_2m,apparent_temperature,precipitation_probability,wind_speed_10m,weathercode"
  );
  ...
  try {
    ...
    if (!res.ok) {
      console.error("Failed to fetch weather:", res.status, res.statusText);
      return null;
    }
    ...
    const snapshot: WeatherSnapshot = {
      ...
      timestamp: now,
      // Open-Meteo returns hourly arrays. For simplicity in this demo, 
      // we stick to the current_weather block mostly, as matching hourly indices requires more logic.
      // If you need feelsLike, you'd match `now` to the index in `data.hourly.time`.
...
export async function getWeatherForLocationAtTime(
  ...
  try {
    const res = await fetch(url.toString());
    if (!res.ok) {
      console.error("Failed to fetch hourly weather:", res.status, res.statusText);
      return null;
    }
